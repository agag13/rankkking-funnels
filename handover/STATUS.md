...
| 7 | Verify AW-17388770213 fires | **Blocked — and the ID does not match** | Page side is ready: `generate_lead` on submit, `lead_thank_you` on /thank-you/, `whatsapp_click` and `phone_click` on those links. Two containers now load, `GTM-NSLJ8GG3` and `GTM-MKJ248GJ`. On the live page the only Google Ads tag that actually loads is **`AW-16610567106`** — the handover's `AW-17388770213` is nowhere on the page. Someone with GTM access has to say which account campaigns will run in and make that conversion fire on `/thank-you/` and on `wa.me` clicks, or conversions land in the wrong account or nowhere. |
