  ...
  return (
    <footer className="border-t border-white/10 bg-navy-950 px-5 py-14">
      <div className="mx-auto grid max-w-6xl gap-10 sm:grid-cols-3">
        <div>
          {logo.wordmark ? (
            <span className="lp-wordmark text-2xl font-extrabold leading-none tracking-tight text-white" aria-label={logo.alt}>
              {logo.wordmark}
            </span>
          ) : (
        ...
        <div>
          ...
          {footer.gstin && <p className="mt-1 text-sm text-slate-400">{footer.gstin}</p>}
        </div>
        <div>
